...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        altron = await e.reply(f"» __WBN SPAM__")
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await altron.edit(f"__🤖 JINDA HU BABE__\n» `{mp} ᴍꜱ'/n » ℳ𝒴 𝒟ℰ𝒱ℰℒ𝒪𝒫ℰℛ~> @Dark_Ravan_01")
...
async def restart(e):
    if e.sender_id in SUDO_USERS:
        await e.reply(f"`RUKO ZARA SABAR KARO...`")
        try:
            await X1.disconnect()
        except Exception:
            pass
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        ok = await event.reply(f"» __AAJA TU BHI AAJA BABY...__")
        target = ""
        ...
        if str(target) in sudousers:
            await ok.edit(f"PEHLE SE SUDO USER HAI BABE !!")
        else:
            if len(sudousers) > 0:
        ...
        await event.reply("»BABE, YE TUMHARE LIYE NHI HAI .")
